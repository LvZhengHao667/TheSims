...
public class Goblin {
    ...
    public Goblin() {
        Random r = new Random();
        int goblinNumber = r.nextInt(2) + roundCounter.getRound()/2 + 4;

        for (int i = 0; i < goblinNumber; i++) {
            name.add("Goblin" + (i + 1));
            hp.add((int) (3 + roundCounter.getRound() * 1.5));
            str.add(1 + roundCounter.getRound());
            ...
            for (i = 0; i < orderString.size(); i++) {
                ...
                if (!heroes.name.contains(orderString.get(i))) {
                    ...
                    for(int j = 0; j <= hp.size()-1 ;j++) {
                        if (hp.get(j) <= 0) {
                            System.out.println(RunArena.RED + name.get(j) + " died." + RunArena.RESET);
